...
from .models import Auction, Order, Chat
...
class GetBuyerChartView(View):
    def get(self, request, buyer_name, *args, **kwargs):
        try:
            buyer = Buyer.objects.get(user= User.objects.get(first_name=buyer_name))
            print(buyer)
            ...
            for order in orders:
                ...
                if amts.get(factory.factory_name):
                    amts[factory.factory_name] = order.amount + amts[factory.factory_name]
                else:
    ...
    def post(self,request, *args, **kwargs):
        ...
        try:
            auction = Auction(
                ...
                employee = SellerFactoryEmployee.objects.get(pk=request.POST.get('employee')),
...
class ShowAuctionHistory(View):
    def get(self, request, factory_name,*args, **kwargs):
        auctions = BackAuction.objects.filter(factory=SellerFactory.objects.get(factory_name=factory_name))
        context = {
            'auctions': auctions
        }
...
class ShowTransactions(View):
    def get(self, request, factory_name, *args, **kwargs):
        try:
            factory = SellerFactory.objects.get(factory_name = factory_name)
            transactions = BackOrder.objects.filter(auction=BackAuction.objects.get(factory=factory))
            hold = []
            n = len(transactions)
            x = n-1
            while x >= 0:
